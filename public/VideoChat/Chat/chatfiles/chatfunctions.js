...
var c_subdir = document.getElementById('chatwindow').getAttribute('data-subdir');  // get the subdir with chatrooms
...
var getchat = function() {return 'chattxt/' + c_subdir +chatroom +'.txt';};        // TXT file with chat content of current room
...
var mypostrequest = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');    // Create the object for AJAX
...
function setHtmlChat(objChat) {
  ...
  if(objChat.time != lastaddedc) {
    ...
    if(nrchats > 0 && objChat.chats[0].chat && objChat.chats[0].chat != '') {
